...
Going forward, both phases run **on every reply**. EventBase handles chat-history events; the chunk-based phase handles every other content type.

```
rearrangeChat
├── Phase A — EventBase (chat history only)
│     └── always invoked; internally skips when no event collection
│         is locked to current chat
│
└── Phase B — Chunk-based content (lorebook / character / document /
              file / URL / wiki / youtube)
      └── always invoked when at least one non-chat collection is
          locked to current chat AND on a supported backend
```

### Routing rules (the if/else the user requested)

```
For every collection in vecthare_collection_registry:

  parsed.type == CHAT
    → handled by Phase A (EventBase). Skip in Phase B.

  parsed.type ∈ { LOREBOOK, CHARACTER, DOCUMENT, FILE, URL, WIKI, YOUTUBE }
    AND collection is locked to current chat
    AND backend ∈ { standard, qdrant }
      → include in Phase B chunk query.
...
- Replace with:
  ...
  // Phase B: chunk-based non-chat content
...
For each candidate collection, drop it unless ALL of these hold:
1. `parseCollectionId(...).type !== COLLECTION_TYPES.CHAT` — chat collections are EventBase territory.
...
Log each rejection with reason (corruption / chat-type / unsupported-backend / not-locked) when `settings.eventbase_debug_logging` is true. Total expected log noise: ≤ N lines per registry entry, no per-message multiplication.
...
Change to:
- If `parsed.type ∈ { LOREBOOK, CHARACTER, DOCUMENT, FILE, URL, WIKI, YOUTUBE }` and there is no lock on the current chat → reject.
- Lorebooks/docs/etc. that are intentionally global (not locked) become invisible to retrieval. This is the user's explicit new policy.
...
### Step 5 — Wire or remove `applySemanticEntriesToPrompt`
...
### Step 6 — Update activation-filter telemetry
...
Files:
- [`tests/world-info-integration.test.js`](../tests/world-info-integration.test.js) — update or remove `applySemanticEntriesToPrompt` tests depending on Step 5 decision.
