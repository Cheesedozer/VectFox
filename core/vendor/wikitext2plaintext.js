/**********************************************************
 * Vendored module: wikitext2plaintext v0.1.0
 * Source: https://www.npmjs.com/package/wikitext2plaintext
 * Author: Dan Kennedy
 * License: MIT (see LICENSE-MIT.txt in the npm package)
 *
 * VectFox modifications:
 * - Converted CommonJS -> ES module export
 * - Replaced the `he` dependency's he.decode() call with
 *   StringUtils.decodeHtmlEntities (same behavior in-browser)
 * No other changes; the parsing rules are verbatim from upstream
 * so output matches the SillyTavern-Fandom-Scraper plugin.
 **********************************************************/
...
wikitext2plaintext.prototype.parse = function(wikitext) {
    ...
    plaintext = StringUtils.decodeHtmlEntities(plaintext);
...
export default wikitext2plaintext;
